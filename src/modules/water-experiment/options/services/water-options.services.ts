...
export class WaterOptionsService implements WaterOptionsServiceTypes {
  ...
  async createWaterOption(waterOption: WaterOptionsTypes) {
    if (
      ...
      throw new ServiceError(
        "Campos obrigatórios da opção ausentes",
        ServiceErrorType.BadRequest,
        undefined,
        ErrorCode.OPTION_MISSING_FIELDS,
      );
    }
  ...
  async getWaterOptionByAnswerNumber(answerNumber: number) {
    const options =
      await this.waterOptionsRepository.findByAnswerNumber(answerNumber);
    if (!options || options.length === 0)
      throw new ServiceError(
        "Opções não encontradas para o número de resposta",
        ServiceErrorType.NotFound,
        undefined,
        ErrorCode.OPTIONS_NOT_FOUND,
      );
    return options;
  }
